...
/*
 * Altered by: Adam Sokolow
 * Date: 5/31/07
 * The mirror class had dependencies on the window being open and 'running' - when the user closed the window, this was an attempt to
 * close the plugin from the Mirror level. However, this created hardware problems because the mirror controller was still trying to send 
 * a reply message and the java program had already been shut down, this created a port miscommunication for the next run, and the
 * subsequent sequence of commands to get 'jumbled.' The mirrors and the microbeam classes no longer have this dependence on the window
 * AND IT IS UP TO THE USER AT THE PLUGIN LEVEL TO HANDLE THE CLOSING OF IMAGEJ WINDOWS - and to handle the closing of the plugin.
 */
